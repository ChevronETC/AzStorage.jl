...
@testset "Container, copy blob to local file" begin
    ...
    c = AzContainer("foo-$r-o", storageaccount=storageaccount, session=session, nthreads=2, nretry=10)
    c = robust_mkpath(c)
    write(c, "foo.txt", "Hello world happy day")
    cp(c, "foo.txt", "foolocal.txt")
    @test read("foolocal.txt", String) == "Hello world happy day"
    rm(c)
    rm("foolocal.txt")
end

@testset "Container, copy large blob to local file" begin
    r = uuid4()
    c = AzContainer("foo-$r-o", storageaccount=storageaccount, session=session, nthreads=2, nretry=10)
    c = robust_mkpath(c)
...
# TODO - Apple seems to product a seg-fault, but I do not have an apple computer to help debug.
if !Sys.iswindows()
    @testset "C token refresh, write" begin
        r = uuid4()
        c = AzContainer("foo-$r-o", storageaccount=storageaccount, session=session, nthreads=4, connect_timeout=2, read_timeout=3, verbose=1)
        c = robust_mkpath(c)
        o = "foo.bin"
